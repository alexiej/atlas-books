...
def print_help() -> None:
    ...
    cmd("python generate.py star-federation",          "parse source → book.md + assets in books-dest/")
    cmd("python generate.py star-federation --no-tts", "skip audio loading")
    cmd("python generate.py --all",                    "process all books")
    cmd("python publish.py star-federation",           "book.md → HTML/EPUB/PDF → public/")
    cmd("python publish.py --all",                     "publish all books in books-dest/")
    cmd("python generate-tts.py star-federation",      "generate MP3 audio (separate step)")
    print()
    print(f"  {bold('PIPELINE')}")
    print(f"    1. {cyan('python generate.py <name>')}   {gray('# parse → books-dest/<name>/book.md')}")
    print(f"    2. {cyan('(edit book.md if needed)')}")
    print(f"    3. {cyan('python publish.py <name>')}    {gray('# book.md → HTML → public/')}")
    print()
    print(f"  {bold('FLAGS')}")
    cmd("--no-tts",             "skip audio entirely")
...
def generate_book(book_dir: Path, do_tts: bool = True) -> bool:
    """Parse book source → write clean book.md + assets to books-dest/<id>/.

    Output (books-dest/<id>/):
      book.md           — normalized, editable markdown (content only)
      config.json       — copy of source config
      cover.jpg/png     — cover image (if present)
      *.mp3             — audio files (split if > 17 MB)
      tts-audio-{lang}.json — audio file map + timing per chapter (if audio present)

    Run publish.py afterwards to generate HTML/EPUB/PDF and copy to public/.
    """
    ...
    # 2. Audio: load TTS sidecar OR copy existing MP3s to dest (with optional split)
    if do_tts:
        ...
        if sidecar_path.exists():
            try:
                ...
                for ch in chapters:
                    entry = sidecar_chs.get(ch.get("id", ""))
                    if entry:
                        mp3 = book_dir / entry["mp3"]
                        if mp3.exists():
                            names = split_mp3_if_large(mp3, dest_dir)
                            ch[f"audio_{lang}"]  = names[0] if len(names) == 1 else names
                            ch[f"timing_{lang}"] = entry.get("timing", [])
                            sidecar_loaded += 1
                if sidecar_loaded:
                    ok(f"Sidecar: {sidecar_loaded} chapters  ({sidecar_path.name})")
            except Exception as e:
                info(f"Sidecar error ({e}) — falling back to plain MP3 copy")
                sidecar_loaded = 0
    ...
    # 3. Write clean book.md to dest
    step("Writing book.md")
    md_content = chapters_to_markdown(chapters)
    (dest_dir / "book.md").write_text(md_content, encoding="utf-8")
    ok(f"book.md  ({len(md_content):,} chars, {len(chapters)} chapters)")

    # 4. Copy config.json to dest
    shutil.copy2(book_dir / "config.json", dest_dir / "config.json")
    ok("config.json")

    # 5. Copy cover image to dest (as file, not base64 — publish embeds base64)
    for name in COVER_NAMES:
        p = book_dir / name
        if p.exists():
    ...
    # 6. MOBI — copy from source if present
    src_mobi = book_dir / "book.mobi"
    if src_mobi.exists():
        shutil.copy2(src_mobi, dest_dir / f"{book_id}.mobi")
        ok(f"MOBI: {book_id}.mobi")

    # 7. Save TTS audio map keyed by the IDs publish.py will see when it re-parses book.md.
    #    The source might give chapters IDs like "ch29" (from epub) while book.md slugifies
    #    titles → "ksiaze".  We re-parse the written markdown to get the stable IDs.
    has_audio = any(ch.get(f"audio_{lang}") for ch in chapters)
    if has_audio:
        reparsed      = md_to_chapters(dest_dir / "book.md")
        tts_map: dict[str, dict] = {}
        for i, ch in enumerate(chapters):
            audio  = ch.get(f"audio_{lang}")
            timing = ch.get(f"timing_{lang}")
            if audio:
                stable_id = reparsed[i].get("id") if i < len(reparsed) else ch.get("id", str(i))
                entry: dict = {"audio": audio}
                if timing:
                    entry["timing"] = timing
                tts_map[stable_id] = entry
        tts_out = dest_dir / f"tts-audio-{lang}.json"
...
def main() -> None:
    ...
    parser.add_argument("--no-tts",      action="store_true", help="Skip MP3 audio loading entirely")
    ...
    kwargs = dict(do_tts=not args.no_tts)
